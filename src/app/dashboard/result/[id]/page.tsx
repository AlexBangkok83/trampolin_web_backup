...
import { ArrowLeft, ExternalLink, Calendar, Target, TrendingUp, BarChart3 } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchResult = async () => {
      try {
        ...
        // Fetch reach data for chart using the same approach as analyze page
        if (data.analysis?.url) {
          ...
          try {
            const reachResponse = await fetch('/api/ads/reach-data', {
              ...
              body: JSON.stringify({ urls: [`https://${data.analysis.url}`] }),
            });
